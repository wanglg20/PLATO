...
class Interaction_Net(layers.Layer):
    ...
    def __init__(self,
        ...
        self.ELU_linear = tf.keras.Sequential(
            [
                layers.Dense(cells_dims, activation='linear'),
            ]
        )
    ...
    def call(self, z, cells):
        ...
        returns:
        interactin:(batch_size, num_slots, slot_dims)
        """
        ...
        cell2in_ij = self.MLP_lambda(tf.concat([h_i, self.ELU_linear(tf.tile(tf.expand_dims(z_b,axis=1),[1,N,1,1]))], axis=-1))
        ...
        # print("interaction:",interaction.shape) #(4,8,512)
...
class InteractionLSTM_Cell(layers.Layer):
    ...
    def call(self, inputs, state):
        ...
        input = tf.expand_dims(inputs, axis=1)                          #(batch_size, 1, num_slots, slot_dims)
        ...
        z_b = self.IN.object_buffer(object_buffer)
        # print("z_b", z_b.shape)
        z_b = self.IN.buffer2int_linear(z_b)
        X = tf.concat((z_b, interaction), axis = -1)
        # print('X', X.shape)                               #(4,8,32)
...
if __name__ == '__main__':
    model = InteractionLSTM(1,6,num_frames=5, use_camera = False)
    inputs = tf.random.normal([1,5,1,6])
    camera = tf.random.normal([1,5,6])
