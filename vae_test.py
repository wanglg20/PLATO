...
def test_vae_step(perception, batch):
    """
    Perform a single test step on image.
    ...
    print("max_value in pred_i:",tf.reduce_max(pred_i))
    print("min value in pred_i:", tf.reduce_min(pred_i))         
...
def test_vae_step_1(perception, batch):
    """
    Perform a single test step on video.
    """
    image = batch['image']
    image = tf.cast(image, tf.float32)
    image = image / 255.0
    ...
    pred_video, pred_mask = pc.video_decoding(perception, latent_code)    # pred_video: (batch_size, t, h, w, 3, num_slots)
    
    print("max_value in video:",tf.reduce_max(image))
    print("min value in video:", tf.reduce_min(image))       
